...
Final things to note: As expected, you should `#include "beat.h"`, and compile it with `beat.c`. You'll also need to link the program to `WinMM` and `Gdi32`. (Old, I know, but they work perfectly fine). Also, in case it isn't obvious to someone out there, `beat.c` is implementing the main function, so you don't need to do it.
